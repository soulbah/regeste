---
paths:
  - 'src/**/*.svelte'
  - 'src/lib/components/**'
---

# UI rules

- **shadcn-svelte first, always.** Before any native HTML interactive element (`button`, `a`, `input`, `select`, `textarea`, `dialog`…), check `src/lib/components/ui/` — if the component exists, use it; if it exists in the registry but not locally, install it with the `add-component` skill. Raw interactive HTML in app code is a task failure. Plain semantic containers (`div`, `section`, `p`, headings) styled with Tailwind are fine.
- Never hardcode colors, radii or fonts — use theme tokens (`bg-background`, `text-muted-foreground`, `border-border`, `font-display`, `rounded-lg`…). Identity lives in `src/routes/layout.css` CSS variables, not in component-level overrides.
- Own the copied components: adapting variants inside `src/lib/components/ui/*` (via `tailwind-variants`) is the expected way to keep our identity — do it there, once, rather than sprinkling class overrides at call sites.
- Svelte 5 runes only (`$state`, `$derived`, `$props`, `$effect`). No `export let`, no `$:`, no legacy stores in new code.
- Shared client state: `.svelte.ts` modules in `src/lib/state/`. Web workers: plain TS in `src/lib/workers/` (runes don't work there), talk to them via Comlink.
- Buttons share one radius: rounded-md (10px), the send button included. Icon buttons are NEVER fully rounded; rounded-full is reserved for passive status chips, dots and avatars (owner rule, 2026-07-10).
- Color signals the exception (owner decision, spec 019): green is the single brand accent (citations, links, focus, confirmations); amber marks data leaving the device (Assisted and My AI egress indicators); Private is uncolored — never decorate the default state. Colored dots appear only on live states (indexing, active egress) and at the moment of egress (pre-send review, egress pill, event logs), never as static decoration — mode names in the picker, pill and settings cards stay undotted (owner, 2026-07-11). No blue mode color.
- Text visible to users goes through i18n (once wired) — never hardcode user-facing French/English strings in components.
- Settings surfaces (owner rule, 2026-07-11): a flat row (label + description left, control right) is for a scalar setting; a card is for a configurable entity with its own state and primary action (state badge + at least two controls — never card a single row). Inside cards, keep the same row grammar. Same content type → same treatment.
- Disabled controls (owner rule, 2026-07-11, exhaustive): every disabled control shows `cursor-not-allowed` — the Button/Checkbox/Switch primitives carry it (`disabled:cursor-not-allowed`, NOT `pointer-events-none`; the global `hover` variant in layout.css excludes disabled elements so hover styles stay dead). Every state-blocked control (empty field, no selection, missing consent, still indexing…) also carries a Tooltip naming the blocker — reason strings live under `disabled.*` in the dictionaries; render the Tooltip.Content conditionally on the blocked state. Busy states (Testing…, Packing…) skip the tooltip: the label already says it.
