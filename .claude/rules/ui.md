...
- Disabled controls (owner rule, 2026-07-11): busy/obvious states (saving, empty required field right above) stay plain disabled. When the blocker lives elsewhere in the app, wrap the disabled button in a `<span class="cursor-not-allowed">` Tooltip trigger that names the blocker — the Button primitive keeps `disabled:pointer-events-none`, so cursor and tooltip must come from the wrapper.
